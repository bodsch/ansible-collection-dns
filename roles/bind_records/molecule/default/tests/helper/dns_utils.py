...
def dig_python(
    ...
    """ """
    bind_cfg = get_vars.get("bind_listen", {}) or {}
    bind_listen_ipv4 = bind_cfg.get("ipv4", [])

    if len(bind_listen_ipv4) == 0:
        bind_listen_ipv4 = {"port": 53, "addresses": ["127.0.0.1"]}

    _adresses = bind_listen_ipv4[0].get("addresses", [])

    local_dns_port = int(bind_listen_ipv4[0].get("port", 53))
    local_dns_address = str(_adresses[0] if _adresses else "127.0.0.1").strip()
